...
public class DocumentStoreResource {
    ...
    public ResponseEntity<List<DocumentStore>> getAllDocumentStores(DocumentStoreCriteria criteria, Pageable pageable) {
        log.debug("REST request to get DocumentStores by criteria: {}", criteria);
        Page<DocumentStore> page = documentStoreQueryService.findByCriteria(criteria, pageable);
