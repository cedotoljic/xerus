...
describe('DocumentStore e2e test', () => {
  ...
  it('should create and save DocumentStores', async () => {
    ...
    await promise.all([
      ...
      documentStoreUpdatePage.setThumbnailInput(absolutePath),
      documentStoreUpdatePage.statusSelectLastOption(),
      documentStoreUpdatePage.setVersionInput('5'),
    ...
    expect(await documentStoreUpdatePage.getThumbnailInput()).to.endsWith(
      fileNameToUpload,
    ...
    const selectedVersionControlled = documentStoreUpdatePage.getVersionControlledInput();
    if (await selectedVersionControlled.isSelected()) {
      await documentStoreUpdatePage.getVersionControlledInput().click();
      expect(await documentStoreUpdatePage.getVersionControlledInput().isSelected(), 'Expected versionControlled not to be selected').to.be
        .false;
    } else {
      await documentStoreUpdatePage.getVersionControlledInput().click();
      expect(await documentStoreUpdatePage.getVersionControlledInput().isSelected(), 'Expected versionControlled to be selected').to.be
        .true;
    }
    expect(await documentStoreUpdatePage.getVersionInput()).to.eq('5', 'Expected version value to be equals to 5');
